...
struct AttributesTest {
    private static let tags: [String] = ["body", "btn", "img", "div", "nav", "section"]

    @Test("Classes are ordered", arguments: tags)
    func classes_areOrdered(tag: String) {
    ...
    func customAttributes_areOrdered(tag: String) {
        let element = Tag(tag) {}
    ...
    func events_areOrdered(tag: String) {
        let element = Tag(tag) {}
    ...
    func styles_areOrdered(tag: String) {
        let element = Tag(tag) {}
            .style(
                .zIndex(1),
                .color(.named("red")),
        ...
        let output = element.markupString()
        #expect(output == "<\(tag) style=\"z-index: 1; color: red; cursor: pointer\"></\(tag)>")
    }

    @Test("Aria attributes are ordered", arguments: tags)
    func ariaAttributes_areOrdered(tag: String) {
        let element = Tag(tag) {}
            .aria(.atomic, "bar")
            .aria(.checked, "qux")
            .aria(.setSize, "foo")
    ...
    func dataAttributes_areOrdered(tag: String) {
        let element = Tag(tag) {}
            .data("foo", "bar")
            .data("baz", "qux")
            .data("qux", "foo")
    ...
    func booleanAttributes_areOrdered(tag: String) {
        let element = Tag(tag) {}
