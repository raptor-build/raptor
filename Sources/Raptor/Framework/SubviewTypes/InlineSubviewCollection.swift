...
struct InlineSubviewCollection: InlineContent, RandomAccessCollection {
    ...
    init(_ children: [any InlineContent] = []) {
        self.elements = children.map(InlineSubview.init)
    }
    ...
    init(_ content: any InlineContent) {
...
private extension InlineSubviewCollection {
    ...
    func flattenedChildren<T: InlineContent>(of html: T) -> [InlineSubview] {
    ...
    func collectFlattenedChildren<T: InlineContent>(_ html: T, into result: inout [InlineSubview]) {
        guard let subviewsProvider = html as? InlineSubviewProvider else {
            if !html.isEmptyContent {
