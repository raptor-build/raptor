...
/// A type-erasing wrapper that can hold any `InlineContent`
/// content while maintaining protocol conformance.
public struct AnyInlineContent: InlineContent {
    ...
    public init(_ content: any InlineContent) {
        var content = content
        attributes.merge(content.attributes)
        content.attributes.clear()

        if let anyHTML = content as? AnyInlineContent {
    ...
    var attributedContent: any InlineContent {
        var wrapped = wrapped
