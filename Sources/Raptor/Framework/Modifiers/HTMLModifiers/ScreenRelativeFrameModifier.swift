//
// ScreenRelativeFrameModifier.swift
// Raptor
// https://raptor.build
...
struct ScreenRelativeFrameHTML<Content: HTML>: HTML {
    ...
    func render() -> Markup {
        ...
        if let minWidth {
            dimensionAttributes.append(styles: .minWidth(.dvw(minWidth.value)))
        }

        if let width {
            dimensionAttributes.append(styles: .width(.dvw(width.value)))
        }

        if let maxWidth {
            dimensionAttributes.append(styles: .maxWidth(.dvw(maxWidth.value)))
        }

        if let minHeight {
            dimensionAttributes.append(styles: .minHeight(.dvh(minHeight.value)))
        }

        if let height {
            dimensionAttributes.append(styles: .height(.dvh(height.value)))
        }

        if let maxHeight {
            dimensionAttributes.append(styles: .maxHeight(.dvh(maxHeight.value)))
        }
        ...
        if let alignment {
            ...
            .style(alignment.flexContainerAlignmentRules)
            .class("screen-relative")
            .render()
        }
...
public extension HTML {
    ...
    func screenRelativeFrame(_ axis: Axis = .horizontal, alignment: Alignment? = nil) -> some HTML {
        var width: Percentage?
        var height: Percentage?
